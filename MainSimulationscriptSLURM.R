...
################################ Simulation start (1 cell) ##########################
    ...
    if(inherits(fit1_W, "try-error")) {
      ...
      MyResult_WLS_CS_est <- matrix(NA, 
                                    ...
                                    ncol = length(ColnamesGeneratorEst("WLS",
                                                                       ...
                                                                       fact,
                                                                       Design[RowOfDesign,3])))
      colnames(MyResult_WLS_CS_est) <- ColnamesGeneratorEst("WLS","CS",
                                                            fact,Design[RowOfDesign,3])
      MyResult_WLS_CS_est[Replication, ] <- MyAnalysisResult_WLS1est
      ...
      MyResult_WLS_CS_err <- matrix(NA, 
                                    ...
                                    ncol = length(ColnamesGeneratorSE("WLS", 
                                                                      ...
                                                                      fact, 
                                                                      Design[RowOfDesign,3])))
      colnames(MyResult_WLS_CS_err) <- ColnamesGeneratorSE("WLS","CS",
                                                           fact,Design[RowOfDesign,3])
      
      MyResult_WLS_CS_err[Replication, ] <- MyAnalysisResult_WLS1err
      
      ### FITINDICES
      FI_WLS_CS <- fitMeasures(fit1_W, c("chisq.scaled","df.scaled", 
                                         "pvalue.scaled", "cfi.scaled", "srmr"))
      MyResult_WLS_CS_FI <- matrix(NA, nrow = Replication, ncol = 5)
      colnames(MyResult_WLS_CS_FI) <- c("chisq.scaled", "df.scaled", 
                                        "pvalue.scaled", "cfi.scaled",
                                        "srmr")
      MyResult_WLS_CS_FI[Replication,] <- FI_WLS_CS
    }
    ...
    if(inherits(fit2_W, "try-error")) {
      ...
      MyResult_WLS_MS_est <- matrix(NA, 
                                    ...
                                    ncol = length(ColnamesGeneratorEst("WLS",
                                                                       ...
                                                                       fact,
                                                                       Design[RowOfDesign,3])))
      colnames(MyResult_WLS_MS_est) <- ColnamesGeneratorEst("WLS","MS",
                                                            fact,Design[RowOfDesign,3])
      MyResult_WLS_MS_est[Replication, ] <- MyAnalysisResult_WLS2est
      ...
      MyResult_WLS_MS_err <- matrix(NA, 
                                    ...
                                    ncol = length(ColnamesGeneratorSE("WLS", 
                                                                      ...
                                                                      fact, 
                                                                      Design[RowOfDesign,3])))
      colnames(MyResult_WLS_MS_err) <- ColnamesGeneratorSE("WLS","MS",
                                                           fact,Design[RowOfDesign,3])
      
      MyResult_WLS_MS_err[Replication, ] <- MyAnalysisResult_WLS2err
      
      ### FITINDICES
      FI_WLS_MS <- fitMeasures(fit2_W, c("chisq.scaled","df.scaled", 
                                         "pvalue.scaled", "cfi.scaled", "srmr"))
      MyResult_WLS_MS_FI <- matrix(NA, nrow = Replication, ncol = 5)
      colnames(MyResult_WLS_MS_FI) <- c("chisq.scaled", "df.scaled", 
                                        "pvalue.scaled", "cfi.scaled",
                                        "srmr")
      MyResult_WLS_MS_FI[Replication,] <- FI_WLS_MS
    }
    ...
    if(inherits(fit1_P, "try-error")) {
      ...
      MyResult_PML_CS_est <- matrix(NA, 
                                    ...
                                    ncol = length(ColnamesGeneratorEst("PML",
                                                                       ...
                                                                       fact,
                                                                       Design[RowOfDesign,3])))
      colnames(MyResult_PML_CS_est) <- ColnamesGeneratorEst("PML","CS",
                                                            fact,Design[RowOfDesign,3])
      MyResult_PML_CS_est[Replication, ] <- MyAnalysisResult_PML1est
      ...
      MyResult_PML_CS_err <- matrix(NA, 
                                    ...
                                    ncol = length(ColnamesGeneratorSE("PML", 
                                                                      ...
                                                                      fact, 
                                                                      Design[RowOfDesign,3])))
      colnames(MyResult_PML_CS_err) <- ColnamesGeneratorSE("PML","CS",
                                                           fact,Design[RowOfDesign,3])
      
      MyResult_PML_CS_err[Replication, ] <- MyAnalysisResult_PML1err
      
      ### FITINDICES
      FI_PML_CS <- fitMeasures(fit1_P, c("chisq.scaled","df.scaled", 
                                         "pvalue.scaled", "cfi.scaled", "srmr"))
      MyResult_PML_CS_FI <- matrix(NA, nrow = Replication, ncol = 5)
      colnames(MyResult_PML_CS_FI) <- c("chisq.scaled", "df.scaled", 
                                        "pvalue.scaled", "cfi.scaled",
                                        "srmr")
      MyResult_PML_CS_FI[Replication,] <- FI_PML_CS
    }
    ...
    if(inherits(fit2_P, "try-error")) { 
      ...
      MyResult_PML_MS_est <- matrix(NA, 
                                    ...
                                    ncol = length(ColnamesGeneratorEst("PML",
                                                                       ...
                                                                       fact,
                                                                       Design[RowOfDesign,3])))
      colnames(MyResult_PML_MS_est) <- ColnamesGeneratorEst("PML","MS",
                                                            fact,Design[RowOfDesign,3])
      MyResult_PML_MS_est[Replication, ] <- MyAnalysisResult_PML2est
      ...
      MyResult_PML_MS_err <- matrix(NA, 
                                    ...
                                    ncol = length(ColnamesGeneratorSE("PML", 
                                                                      ...
                                                                      fact, 
                                                                      Design[RowOfDesign,3])))
      colnames(MyResult_PML_MS_err) <- ColnamesGeneratorSE("PML","MS",
                                                           fact,Design[RowOfDesign,3])
      
      MyResult_PML_MS_err[Replication, ] <- MyAnalysisResult_PML2err
      
      
      ### FITINDICES
      FI_PML_MS <- fitMeasures(fit2_P, c("chisq.scaled","df.scaled", 
                                         "pvalue.scaled", "cfi.scaled", "srmr"))
      MyResult_PML_MS_FI <- matrix(NA, nrow = Replication, ncol = 5)
      colnames(MyResult_PML_MS_FI) <- c("chisq.scaled", "df.scaled", 
                                        "pvalue.scaled", "cfi.scaled",
                                        "srmr")
      MyResult_PML_MS_FI[Replication,] <- FI_PML_MS
    }
  ...
  # save the results
    write.csv(MyResult_WLS_CS_est, 
              file = paste("WLS_CS_est", "Row", RowOfDesign, ".csv" , sep =""))
    
    write.csv(MyResult_WLS_CS_err,
              file = paste("WLS_CS_err", "Row", RowOfDesign, ".csv" , sep =""))
    
    write.csv(MyResult_WLS_CS_FI,
              file =paste("FI_WLS_CS", "Row", RowOfDesign,".csv" , sep =""))
    
    write.csv(MyResult_WLS_MS_est,
              file = paste("WLS_MS_est", "Row", RowOfDesign, ".csv" , sep =""))
    
    write.csv(MyResult_WLS_MS_err,
              file = paste("WLS_MS_err", "Row", RowOfDesign,".csv" , sep =""))
    
    write.csv(MyResult_WLS_MS_FI,
              file =paste("FI_WLS_MS", "Row", RowOfDesign, ".csv" , sep =""))
    
    write.csv(MyResult_PML_CS_est,
              file = paste("PML_CS_est", "Row", RowOfDesign ,".csv" , sep =""))
    
    write.csv(MyResult_PML_CS_err,
              file = paste("PML_CS_err", "Row", RowOfDesign ,".csv" , sep =""))
    
    write.csv(MyResult_PML_CS_FI,
              file =paste("FI_PML_CS", "Row", RowOfDesign,".csv" , sep =""))
    
    write.csv(MyResult_PML_MS_est,
              file = paste("PML_MS_est", "Row", RowOfDesign,".csv" , sep =""))
    
    write.csv(MyResult_PML_MS_err,
              file = paste("PML_MS_err", "Row", RowOfDesign,".csv" , sep =""))
    
    write.csv(MyResult_PML_MS_FI,
              file =paste("FI_PML_MS", "Row", RowOfDesign, ".csv" , sep =""))
